...
class Cell extends Context {
  constructor(
    ...
    if (column.rule && column.rule.immediate === false) return; // Инициализация редактора не требует проверки
    this.validate();
  }
  ...
  isInHorizontalAutofill(mouseX, mouseY) {
    return (
      ...
      mouseX < this.grid.width - this.grid.fixedRightWidth + 4 // Совместимость с крайним правым контактом автозаполнения будет недействительна, поскольку он отображается на скроллере
    );
  }
  ...
  // Для данных выпадающего типа метка выводится наружу, а значение фактически сохраняется, поэтому при обновлении этого типа данных нужно делать преобразование
  ...
  draw() {
    ...
    const x =
      this.fixed === "right" // right
        ? width -
          (tableWidth - this.x - this.width) -
          this.width -
          verticalScrollerSize
        : this.fixed === "left" // left
        ? this.x
        : this.x + scrollX;
    ...
    if (this.dataType === 'children') {
      // this.width
      painter.drawCellAffixIcon('arrow', x, y, 30, this.height-2, {
        color: '#bbbec4',
        fillColor: this.readonly ? READONLY_COLOR : fillColor,
        textAlign: 'left'
      })
