...
class DataGrid {
  ...
  /**
   * Передать данные из ячеек холста в редактор
   */
  putCell() {
  ...
  updateColumns(columns) {
    ...
    // Вычисляем межстрочный colspan и межстолбцовый rowspan составного заголовка, который используется для рендеринга заголовка
    this.headers = calCrossSpan(columns, maxHeaderRow)

     // Получаем заголовок конечного узла для рендеринга данных
